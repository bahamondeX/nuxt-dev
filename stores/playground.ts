...
export const usePlaygroundStore = defineStore("playground", () => {
  ...
  if (import.meta.client) {
    async function init() {
      ...
      Object.entries(filesRaw).forEach(([path, content]) => {
  ...
  async function launchInstallProcess(wc: WebContainer, signal: AbortSignal) {
    ...
    const installExitCode = await spawn(wc, "pnpm", [
      "install",
      "--prefer-offline",
    ]);
    if (signal.aborted) return;
  ...
  async function _updateOrCreateFile(filepath: string, content: string) {
    ...
    if (file) {
      if (file.read() !== content) await file.write(content);
      return file;
    } else {
  ...
  async function mount(map: Record<string, string>, templates = filesTemplate) {
    ...
    await Promise.all([
      ...
      ...Object.entries(objects).map(async ([filepath, content]) => {
  ...
  function createFile(path: string, content: string) {
    ...
    files.set(path, new VirtualFile(path, content, webcontainer.value!));
  }

  async function deleteFile(path: string) {
    await webcontainer.value?.fs.rm(path);
  }
