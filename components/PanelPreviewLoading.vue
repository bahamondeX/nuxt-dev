...
<template>
  ...
  >
    ...
    <div
      v-else
      grid="~ cols-[max-content_1fr] gap-2 items-center justify-center"
    >
      <div :class="getStatusIcon('init')" />
      <span :class="getTextClass('init')">{{
        $t("steps.initializing-webcontainer")
      }}</span>
      <div :class="getStatusIcon('mount')" />
      <span :class="getTextClass('mount')">{{
        $t("steps.mounting-files")
      }}</span>
      <div :class="getStatusIcon('install')" />
      <span :class="getTextClass('install')">{{
        $t("steps.installing-dependencies")
      }}</span>
      <div :class="getStatusIcon('start')" />
      <span :class="getTextClass('start')">{{
        $t("steps.starting-nuxt-server")
      }}</span>
      <div :class="getStatusIcon('polling')" />
      <span :class="getTextClass('polling')">{{
        $t("steps.waiting-for-nuxt-to-ready")
      }}</span>
    </div>
  </div>
</template>
