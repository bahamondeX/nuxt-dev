...
function move(delta: number) {
  ...
  if (selected.value < 0) selected.value = commands.commandsResult.length - 1;
  if (selected.value >= commands.commandsResult.length) selected.value = 0;
}

function runCommand(command: Command) {
  if (command.handler) command.handler();
  else if (command.to) router.push(command.to);
...
function getTitle(command: Command) {
  if (typeof command.title === "function") return command.title();
...
<template>
  ...
  >
    <div absolute inset-0 z--1 bg-black:75 @click="commands.isShown = false" />
    <div
      border="~ base rounded"
      h-100
      w-200
